...
const Farmdataupload = () => {
  ...
  // Handle image selection with validation for file size and type
  const handleImageChange = (e) => {
    ...
    if (selectedFile) {
      ...
      if (!validImageTypes.includes(selectedFile.type)) {
        ...
        setFormData({
          ...formData,
          image: null,  // Reset image if file type is invalid
        });
      } else if (selectedFile.size > maxSize) {
        setFileError('File size is too large. Please select a file less than 10MB.');
        setFormData({
          ...formData,
          image: null,  // Reset image if file size is too large
        });
      } else {
        setFileError('');
        setFormData({
          ...formData,
          image: selectedFile, // Save the selected file
        });
      }
    }
  };
    ...
    <div className="header pb-8 pt-4 pt-lg-8 "
      ...
      <Container>
        <Row className="justify-content-center">
          <Col lg="6" md="8" sm="12">
            <Card className=" border-3 mt-5 p-4" style={{ backgroundColor: "transparent" }}>
